...
void draw() {
  ...
  //Iterate through all the houses and draw them 
  for (int i = 0; i < houses.length; i++) {
    houses[i].drawHouse();
  }

  //Make a random house a random size each time through draw
  houses[int(random(houses.length))].houseSize = int (random (0, 20));

  // When you mouseover the 0th House, all the houses get big
  if (houses[0].isMouseOver()) {
    for (int i = 0; i < houses.length; i++) {
      houses[i].setSize(100);
    }
  }
}
